...
### 5. 💎 多维度 Token 消耗统计看板与真实费用换算 (Token Analytics & Cost Engine)
- **全局 6 大指标卡片**：累计总消耗 (Total Tokens)、**💰 预估总支出 (¥ 人民币 / $ 美元)**、Prompt (Input)、Completion (Output)、**今日消耗与今日费用**、请求总数；
- **📈 7/14/30 天消耗与费用趋势图**：响应式可视化柱状图，鼠标悬浮即时显示每日详细 Token 吞吐、Prompt/Completion 构成、预估费用与请求频次；
- **🍩 模型消耗分布 (Top Models)**：按模型聚合消耗占比，展示彩色渐变进度条、模型 Token 总量及折算费用；
- **🏷️ 渠道 & 🔑 Key 细分维度**：精确归因到每个渠道与每个脱敏 API Key 的消耗与费用分布；
- **📥 一键导出 CSV 账单报表**：支持随时一键下载全量 Token 消耗明细（UTF-8 BOM 编码，Excel 打开不乱码）；
- **零延迟流式提取与便携持久化**：透明解析 Anthropic SSE `message_delta` 与 OpenAI `usage` 结构体，优雅停机强制落盘，历史数据永不丢失。
...
### 9. 🔄 欠费/限流自动静默救急与智能熔断冷却 (Smart Failover & Circuit Breaker)
- **⚡ 故障渠道智能熔断冷却 (Circuit Breaker)**：当渠道连续 3 次报错或超时，**自动进入 30 秒熔断冷却期**。期间后续请求**直接秒级跳过该渠道**，避免客户端傻等超时卡顿，链路中清晰标明 `[渠道名 (⚡熔断中，剩余 {n}s)]`；冷却结束后自动探测复苏；
...
### 13. 🌐 独立渠道上游网络代理 (Per-Channel Upstream Proxy)
- **多渠道代理与直连混用**：允许为指定渠道单独配置 HTTP / SOCKS5 代理（如 `http://127.0.0.1:7890` 或 `socks5://127.0.0.1:10808`）；
- **官方 API 与国内中转并存**：实现海外需翻墙的 Anthropic / OpenAI 官方直连渠道走本地代理，而国内直连中转站走直连，同一网关内无缝混用；
- **内置快捷预设**：弹窗内置 Clash (7890)、v2ray (10808)、Clash Verge (7897) 等常见本地代理端口一键填入。

### 14. 💾 渠道配置一键备份、导出与导入还原 (Backup, Export & Restore)
- **全量一键导出**：支持一键将全部渠道配置（含多 Key、别名重命名映射、自定义特征请求头、代理配置等）导出为标准 JSON 备份文件；
- **极速导入迁移**：换电脑、重装系统或多设备协同，一键上传 JSON 文件即可一键恢复全量配置；
- **智能追加合并**：支持以「追加合并」模式导入，保留当前配置同时合并新渠道。

### 15. 🔒 网关全局安全与局域网访问鉴权 (Gateway Auth Token)
- **局域网共享保护**：在设置中心支持开启「网关请求访问鉴权」，防止局域网内其他人员未授权盗刷你的 API 额度；
